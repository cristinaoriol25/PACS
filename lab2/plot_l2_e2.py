...
def main(args):
    ...
    df = df[['method', 'N','clk(ini)', 'clk(mult)', 'User(s)', 'Real(s)','System(s)']] #
    ...
    dif = ini_plus_mult - (df['User(s)'] + df['System(s)'])# df['User(s)']
    print('avg,median,std diff:',np.mean(dif), np.median(dif), np.std(dif))

    # Init overhead wrt N for each method:
    ini = df[df['method']=="sequential"]['clk(ini)'].values
    i_oh_seq = ini / (ini + df[df['method']=="sequential"]['clk(mult)'].values)
    ini = df[df['method']=="my_eigen_matmult"]['clk(ini)'].values
    i_oh_eigen = ini / (ini + df[df['method']=="my_eigen_matmult"]['clk(mult)'].values) # TODO: en serio la inicializacion es tan lenta??
    # Plot it:
    fig, ax = plt.subplots()
    ax.scatter(x=df[df['method']=="sequential"]['N'], y=i_oh_seq, label="sequential")
    ax.scatter(x=df[df['method']=="my_eigen_matmult"]['N'], y=i_oh_eigen, label="eigen")
    ...
    ax.legend()
    ax.set_xlabel("N")
    ax.set_ylabel("relative initialization overhead")
    ax.set_title("initialization overhead - N for each method")
    ax.grid()
    plt.savefig('gettimeofday-ini-overhead.png')
    ...
    ax.set_xlabel("N")
    ax.set_ylabel("t (s)")
    ax.set_title("gettimeofday time - N for each method")
    ax.grid()
    plt.savefig('gettimeofday-ini-times.png')
    #
    # axes = plt.gca()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--log', help='log',
                        type=str, default="src/p2-logs/gettimeofday.log")
